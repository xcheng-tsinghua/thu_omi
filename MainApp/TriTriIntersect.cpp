...
int coplanar_tri_tri_dbl(double N[3],double V0[3],double V1[3],double V2[3],
...
inline void isect2_dbl(double VTX0[3],double VTX1[3],double VTX2[3],double VV0,double VV1,double VV2,
...
inline int compute_intervals_isectline_dbl(double VERT0[3],double VERT1[3],double VERT2[3],
                                       ...
                                       double isectpoint0[3],double isectpoint1[3])
{
    ...
    {                                                    
        /* here we know that D0D2<=0.0 */                  
        /* that is D0, D1 are on the same side, D2 on the other or on the plane */
        isect2_dbl(VERT2,VERT0,VERT1,VV2,VV0,VV1,D2,D0,D1,isect0,isect1,isectpoint0,isectpoint1);
    } 
    else if(D0D2>0.0f)                                   
    {                                                   
        /* here we know that d0d1<=0.0 */             
        isect2_dbl(VERT1,VERT0,VERT2,VV1,VV0,VV2,D1,D0,D2,isect0,isect1,isectpoint0,isectpoint1);
    }                                                  
    else if(D1*D2>0.0f || D0!=0.0f)   
    {                                   
        /* here we know that d0d1<=0.0 or that D0!=0.0 */
        isect2_dbl(VERT0,VERT1,VERT2,VV0,VV1,VV2,D0,D1,D2,isect0,isect1,isectpoint0,isectpoint1);   
    }                                                  
    else if(D1!=0.0f)                                  
    {                                               
        isect2_dbl(VERT1,VERT0,VERT2,VV1,VV0,VV2,D1,D0,D2,isect0,isect1,isectpoint0,isectpoint1); 
                                     ...
                                     double isectpt1[3],double isectpt2[3])
{
    ...
    /* coplanarity robustness check */
#if USE_EPSILON_TEST==TRUE
    if(fabs(du0)<EPSILON_DBL) du0=0.0;
    if(fabs(du1)<EPSILON_DBL) du1=0.0;
    if(fabs(du2)<EPSILON_DBL) du2=0.0;
#endif
    du0du1=du0*du1;
    du0du2=du0*du2;
    ...
    /* put .,,V1,V2 into plane equation 2 */
    dv0=DOT(N2,V0)+d2;
    dv1=DOT(N2,V1)+d2;
    dv2=DOT(N2,V2)+d2;

#if USE_EPSILON_TEST==TRUE
    if(fabs(dv0)<EPSILON_DBL) dv0=0.0;
    if(fabs(dv1)<EPSILON_DBL) dv1=0.0;
    if(fabs(dv2)<EPSILON_DBL) dv2=0.0;
#endif
    ...
    /* compute interval for triangle 1 */
    *coplanar=compute_intervals_isectline_dbl(V0,V1,V2,vp0,vp1,vp2,dv0,dv1,dv2,
        dv0dv1,dv0dv2,&isect1[0],&isect1[1],isectpointA1,isectpointA2);
    if(*coplanar) return coplanar_tri_tri_dbl(N1,V0,V1,V2,U0,U1,U2);     


    /* compute interval for triangle 2 */
    compute_intervals_isectline_dbl(U0,U1,U2,up0,up1,up2,du0,du1,du2,
        du0du1,du0du2,&isect2[0],&isect2[1],isectpointB1,isectpointB2);

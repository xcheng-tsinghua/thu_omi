...
*/
{
	...
	printf(PROG_ID_STR);//打印"\nACO algorithms for the TSP, v1.03\n"
	set_default_parameters();//设置默认参数
	setbuf(stdout, NULL);//设置标准输出为空
						 //parse_commandline(argc, argv);//解析命令行参数

	strcpy(name_buf, fname);
	read_parameters_aco(name_buf);

	assert(max_tries <= MAXIMUM_NO_TRIES);//最大尝试次数小于该值
	...
	instance.distance = compute_distances();//计算出距离矩阵
	printf(" .. done\n");
	write_params();//记录下参数
	if (comp_report)
...
*/
{
    ...
    while ( strcmp("NODE_COORD_SECTION", buf) != 0 ) {
		if ( strcmp("NAME", buf) == 0 ) {
			...
			fscanf(tsp_file, "%s", buf);
			TRACE ( printf("%s\n", buf); );
			if ( strcmp("EUC_3D", buf) == 0 ) {
			distance = round_distance;
			}
			else if ( strcmp("CEIL_3D", buf) == 0 ) {
    ...
    if( (nodeptr = malloc(sizeof(struct point) * n)) == NULL )
		...
		for ( i = 0 ; i < n ; i++ ) {
			fscanf(tsp_file,"%ld %lf %lf %lf", &j, &nodeptr[i].x, &nodeptr[i].y ,&nodeptr[i].z);
		}
    }
...
static void fprintf_parameters (FILE *stream)
{
...
void print_default_parameters(FILE *fname) 
/*    
...
void writeTour(char *fname) {
	FILE *tour;
	int i;

	if ((tour = fopen(fname, "w")) == (FILE*)NULL) {
		eprintf("文件%s打开失败", fname);
	}

	fprintf(tour, "NAME : %s\n", fname);
	fprintf(tour, "COMMENT : Found by ACO\n");

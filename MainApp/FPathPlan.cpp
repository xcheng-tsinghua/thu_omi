...
void FPathPlan::ComputeNoCollisionPath(FIpPntList& ipPntList, FIpPathList& pathlist)
{
	...
	double prob_radius = 0.5 * gWindow->GetMainDoc()->getProbeDia();  // 测针红宝石球半径
	...
	if (abs(prob_radius) > 100)
	{
		...
		double probe_diameter = QInputDialog::getDouble(0, QString::fromLocal8Bit("输入测针红宝石球直径"), QString::fromLocal8Bit("测针红宝石球直径:"), 6, 0, 100, 5);
